...
function postData(state = {}, action) {
  switch (action.type) {
    ...
    case actionTypes.SUBMIT_COMMENT: {
      const newState = { ...state };
      const array = newState[action.item.id].comments;
      array.push(action.id);
      newState[action.item.id].comments = array;
      return {
        ...newState
      };
    }
...
function department(state = {
  ...
  10010101: {
    id: 10010101, name: '总经办', child: [], user: [2018003, 2018002]
  },
  10010102: {
    id: 10010102, name: '财务部', child: [], user: [2018003, 2018005, 2018001, 2018002]
  },
...
function user(state = {
  ...
  switch (action.type) {
    case actionTypes.SUBMIT_USER_NAME: {
      const newState = { ...state };
      for (const i in action.arr) {
        newState[action.arr[i]].isSeleted = true;
      }
      return newState;
    }
    default:
